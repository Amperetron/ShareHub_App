...
export default function LocationPickerModal({ visible, onClose, onSelect, title }: Props) {
  ...
  const [mapSelected, setMapSelected] = useState<{ lat: number; lng: number; address: string } | null>(null);
  ...
  const handleMapMessage = (event: any) => {
    try {
      ...
      if (data.type === 'mapClick') {
        setMapSelected({ lat: data.lat, lng: data.lng, address: data.displayName });
      }
    } catch {}
  };
...
map.on('click',function(e){
  ...
  fetch('https://nominatim.openstreetmap.org/reverse?format=json&lat='+lat+'&lon='+lng)
    ...
    .then(function(d){
      var name=d.display_name||'Unknown location';
      window.ReactNativeWebView.postMessage(JSON.stringify({type:'mapClick',lat:lat,lng:lng,displayName:name}));
    })
    .catch(function(){
      window.ReactNativeWebView.postMessage(JSON.stringify({type:'mapClick',lat:lat,lng:lng,displayName:'Unknown location'}));
    });
});
</script>
</body>
      ...
      <View style={styles.container}>
          ...
          <View style={styles.mapContainer}>
            ...
            <View style={styles.hintBar}>
              <Text style={styles.hint}>
                {mapSelected ? 'Tap elsewhere to adjust' : 'Click on the map to select'}
              </Text>
            </View>
          </View>
        ) : (
        ...
        <View style={[styles.footer, { paddingBottom: insets.bottom + 20 }]}>
          <Pressable
            ...
            onPress={() => {
              if (mode === 'map' && mapSelected) {
                onSelect({ lat: mapSelected.lat, lng: mapSelected.lng, address: mapSelected.address });
